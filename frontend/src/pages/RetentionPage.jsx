...
export default function RetentionPage() {
  ...
  const openWhatsApp = (member) => {
    ...
    const msg = encodeURIComponent(`¡Hola ${member.fullName.split(' ')[0]}! Te escribimos desde ${currentOrgName}. Tu membresía está por vencer, ¿querés renovarla?`);
      ...
      <div className="dashboard-grid">
        ...
        <div className="card">
          <div className="table-header">
            ...
            <span className="badge badge-warning badge-count">{(analytics.expiring_soon || []).length}</span>
          </div>
          <div style={{ maxHeight: 400, overflowY: 'auto', padding: '0 1rem 1rem' }}>
            {(!analytics.expiring_soon || analytics.expiring_soon.length === 0) ? (
              ...
              const diff = Math.ceil((new Date(m.membershipEnd) - new Date()) / (1000 * 60 * 60 * 24));
              return (
                <div key={m.id} className="checked-in-item" style={{ marginBottom: '0.5rem', opacity: isFetching ? 0.7 : 1, transition: 'opacity 0.2s' }}>
                  ...
                  <div className="member-info" style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 'var(--font-size-xs)', color: 'var(--text-muted)' }}>Vence: {formatDate(m.membershipEnd)}</div>
                  </div>
                  <div className={`days-countdown ${diff <= 1 ? 'days-danger' : 'days-warning'}`}>{diff}d</div>
                  {m.phone && (
                    <button className="action-btn-quick action-btn-whatsapp" onClick={() => openWhatsApp(m)} title="WhatsApp"><Icon name="messageCircle" size="1em" /></button>
                  )}
                </div>
              );
            })}
        ...
        <div className="card">
          <div className="table-header">
            <h3 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Icon name="alertTriangle" size="1em" /> En Riesgo</h3>
            <span className="badge badge-error badge-count">{(analytics.at_risk || []).length}</span>
          </div>
          <div style={{ maxHeight: 400, overflowY: 'auto', padding: '0 1rem 1rem' }}>
