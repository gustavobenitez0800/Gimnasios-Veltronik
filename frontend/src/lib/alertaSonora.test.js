...
describe('la X del socio vencido', () => {
  it('⭐ baja la música y DESPUÉS suena: si sonara primero, la tapa la música', () => {
    expect(sonarVencido(10_000)).toBe(true);
  ...
  it('dos vencidos seguidos suenan una sola vez', () => {
    expect(sonarVencido(10_000)).toBe(true);
    expect(sonarVencido(10_500)).toBe(false);
    expect(sonarVencido(12_000), 'pasado el momento, vuelve a sonar').toBe(true);
  ...
  it('en el portal web no suena: el dueño lo mira desde otro lado', () => {
    ...
    expect(sonarVencido(10_000)).toBe(false);
  ...
  it('una PC sin audio no rompe nada', () => {
    ...
    expect(() => { sonarVencido(10_000); vi.advanceTimersByTime(150); }).not.toThrow();
  });
});
