...
export default class InsightsService {
  ...
  getPaymentAlerts(members) {
    ...
    members.forEach((member) => {
      if (!member.membershipEnd) return;
      ...
      if (daysToExpire <= 0) {
        alerts.push({
          type: 'expired',
          member,
          message: `${member.fullName} - Membresía vencida`,
          priority: 'high',
        ...
        alerts.push({
          type: 'urgent',
          member,
          message: `${member.fullName} - Vence en ${daysToExpire} día${daysToExpire > 1 ? 's' : ''}`,
          priority: 'high',
        ...
        alerts.push({
          type: 'warning',
          member,
          message: `${member.fullName} - Vence en ${daysToExpire} días`,
          priority: 'medium',
          daysRemaining: daysToExpire,
        });
      }
  ...
  generateDailyInsights(data) {
    ...
    const monthlyIncome = payments
      .filter((p) => new Date(p.paymentDate) >= startOfMonth && p.status === 'paid')
